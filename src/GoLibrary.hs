...
getStone game@(Game b bz pl lm mh sb sw) point = findKey point b
...
turnPlayer game@(Game b bz pl lm mh sb sw) = case lm of
                                               Pass stone -> getOppositeStone stone
...
removePrisoners game point prison_stone
    | length removeAll == 1 && isKo game = updateScore (labelKo (removeStones game removeAll) (removeAll !! 0)) 1 attack_stone
...
unlabelAllKo game@(Game b bz pl lm mh sb sw) point stone = Game{
    ...
    scoreBlack = sb,
    scoreWhite = sw
} where ko_points = L.filter (\(p, s) -> s == Ko) (assocs b)
...
bfsSearchDead game@(Game b bz pl lm mh sb sw) point@(Point x y) prison_stone seen_points
    ...
    | findKey point b == Empty                   = Nothing:seen_points
    | findKey point b == Ko                      = Nothing:seen_points
    | findKey point b /= prison_stone            = seen_points
...
countTerritory game = (blackTerritory, whiteTerritory)
    where
        blackTerritory = length $ Prelude.filter (\x -> x == Black) territory
        whiteTerritory = length $ Prelude.filter (\x -> x == White) territory
...
decideTerritory game@(Game b bz pl lm mh sb sw) point
    | findKey point b == Black  = Black
    | findKey point b == White  = White
...
countOneDirection game@(Game b bz pl lm mh sb sw) point@(Point x y) (xx, yy) stone
    | findKey newpoint b == Nothing                = 0
    | findKey newpoint b == stone                  = 1
    | findKey newpoint b == getOppositeStone stone = 0
